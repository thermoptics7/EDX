...
def in_parentheses(searchString):
    leftParenIndex = searchString.find("(")
    rightParenIndex = searchString.find(")")

    if leftParenIndex >= 0:
        if rightParenIndex >= 0:
            return searchString[leftParenIndex + 1:rightParenIndex]
        else:
            return ""
    else:
        return ""
...
#print(in_parentheses("(hey hi hello)"))
print(in_parentheses("And here's the other ( one!"))